...
import { BrainCircuit, Plus, Database, ShieldCheck, Trash2, Edit3, Sparkles, Link2, Search, Filter, Loader2, X } from 'lucide-react';
            ...
            {isAdding && (
                <div className="bg-dark-800 p-10 rounded-[2.5rem] border border-google-blue/30 shadow-2xl space-y-8 animate-in slide-in-from-top-4">
                    <div className="flex justify-between items-center">
                        <h3 className="text-xl font-black text-white italic">Neural Knowledge Injection</h3>
                        <button onClick={() => setIsAdding(false)} className="p-2 hover:bg-dark-700 rounded-xl transition-colors"><X size={20} /></button>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredFacts.map(fact => (
                    <div key={fact.id} className="bg-dark-800 p-8 rounded-[2.5rem] border border-dark-700 shadow-xl group hover:border-google-blue/30 transition-all flex flex-col h-full relative overflow-hidden">
                        ...
                        {/* Fix: Explicit check for agentAffinities existence to satisfy TypeScript type narrowing */}
                        {fact.agentAffinities && fact.agentAffinities.length > 0 && (
                            <div className="mb-6 flex flex-wrap gap-2 relative z-10">
                                {fact.agentAffinities.map((aff: string) => (
                        ...
                        <div className="pt-6 border-t border-dark-700 flex justify-between items-center opacity-40 group-hover:opacity-100 transition-opacity relative z-10">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button 
                                    ...
                                    className="p-2 text-gray-500 hover:text-google-red transition-colors disabled:opacity-50"
                                >
                                    {isDeleting === fact.id ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
                                </button>
                            </div>
                        </div>
                    </div>
