...
import { ShieldAlert, ShieldCheck, Lock, Eye, RefreshCw, Terminal, Search, AlertTriangle, Loader2, FileWarning, Zap, ChevronRight } from 'lucide-react';

const SecurityLab = () => {
    const [isScanning, setIsScanning] = useState(false);
    const [report, setReport] = useState<string | null>(null);
    const [threatLevel, setThreatLevel] = useState<'LOW' | 'MEDIUM' | 'CRITICAL'>('LOW');
    const [vulnerabilities] = useState([
        { id: 1, level: 'low', type: 'Worktree Divergence', status: 'monitored', desc: 'Possible symlink bridge between design and seo worktrees detected.' },
        { id: 2, level: 'critical', type: 'Trust Boundary Leak', status: 'vulnerable', desc: 'Agent environment contains unmasked session tokens in design sub-process.' }
    ]);
    ...
    const runNeuralAudit = async () => {
        ...
        try {
            ...
            if (data.report.toLowerCase().includes('critical') || data.report.toLowerCase().includes('vulnerability')) {
                ...
                <div className="lg:col-span-1 space-y-8">
                    <div className="bg-dark-800 p-8 rounded-[2.5rem] border border-dark-700 shadow-2xl space-y-8 flex flex-col h-full">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="bg-google-red/10 text-google-red border border-google-red/20 px-3 py-1 rounded-full text-[8px] font-black uppercase animate-pulse">Scanning Active</span>
                        </div>
                        ...
                        <div className="space-y-4 flex-1">
                            {vulnerabilities.map(vuln => (
                                <div key={vuln.id} className="p-6 bg-dark-900 rounded-3xl border border-dark-700 hover:border-google-red/30 transition-all group relative overflow-hidden">
                                    <div className="absolute top-0 right-0 w-20 h-20 bg-google-red/5 blur-2xl -mr-10 -mt-10" />
                                    <div className="flex justify-between items-start mb-4 relative z-10">
                                        <div className={`p-3 rounded-xl shadow-lg ${vuln.level === 'critical' ? 'bg-google-red/20 text-google-red' : 'bg-google-yellow/20 text-google-yellow'}`}>
                                            <ShieldAlert size={20} />
                                        </div>
                                        <div className="text-right">
                                            <div className="text-[8px] font-black uppercase text-gray-600 mb-1 tracking-widest">Risk Index</div>
                                            <div className={`text-[10px] font-black uppercase ${vuln.level === 'critical' ? 'text-google-red' : 'text-google-yellow'}`}>{vuln.level}</div>
                                        </div>
                                    </div>
                                    ...
                                    <button className="w-full mt-6 py-4 bg-dark-800 text-[10px] font-black uppercase text-gray-400 rounded-xl hover:text-white hover:bg-google-red/20 transition-all border border-dark-700 relative z-10">
                                        Deploy Patch Protocol
                                    </button>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="p-6 bg-dark-900 rounded-3xl border border-dark-700 flex items-center gap-4">
                            <div className={`w-3 h-3 rounded-full animate-ping ${threatLevel === 'LOW' ? 'bg-google-green' : 'bg-google-red'}`} />
                            <div className="text-[10px] font-black uppercase tracking-widest text-gray-500">Global Threat Level: <span className={threatLevel === 'LOW' ? 'text-google-green' : 'text-google-red'}>{threatLevel}</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 flex flex-col h-full">
                    <div className="bg-dark-800 rounded-[2.5rem] border border-dark-700 shadow-2xl flex flex-col flex-1 overflow-hidden relative">
                        <div className="p-8 border-b border-dark-700 flex justify-between items-center bg-dark-900/40">
                            ...
                            {report && <span className="text-[9px] font-black uppercase text-google-green bg-google-green/10 border border-google-green/30 px-4 py-1.5 rounded-full flex items-center gap-2"><ShieldCheck size={12} /> Sync Integrity Validated</span>}
                        </div>
                        ...
                        <div className="p-8 border-t border-dark-700 bg-dark-900/50 grid grid-cols-2 gap-8 shrink-0">
                            ...
                            <div className="flex items-center gap-5">
                                ...
                                <div>
                                    <div className="text-[9px] font-black uppercase text-gray-500 tracking-widest">Perimeter Integrity</div>
                                    <div className="text-sm font-black text-white">NOMINAL - 99.4%</div>
                                </div>
                            </div>
                        </div>
                    </div>
